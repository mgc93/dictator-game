...
jsPsych.plugins['html-slider-response-dg-hover'] = (function() {

  var plugin = {};

  plugin.info = {
    name: 'html-slider-response-dg-hover',
    description: '',
    parameters: {
      stimulus: {
  ...
  plugin.trial = function(display_element, trial) {
    ...
    html += '<div class="sliderValue">';
    html += '<span>50</span>';
    html += '</div>';

    html += '<div class="field">';
    html += '<input type="range" value="'+trial.slider_start+'" min="'+trial.min+'" max="'+trial.max+'" step="'+trial.step+'" style="width: 100%;"></input>';
    //html += '<span id="slidertitle"></span>';
    html += '</div>';
    ...
    for(var j=0; j < trial.labels_top.length; j++){
      ...
      html += '<span id="labels";>'+trial.labels_top[j]+'</span>';
      html += '</div>'
    }
    for(var j=0; j < trial.labels_bottom.length; j++){
      ...
      html += '<span id="labels";>'+trial.labels_bottom[j]+'</span>';
      html += '</div>'
    }
    html += '</div>';
    html += '</div>';
    ...
    // var slider = display_element.querySelector('#jspsych-html-slider-response-response');
    // var slidertitle = display_element.querySelector('#slidertitle');
    // var container = display_element.querySelector('.jspsych-content-wrapper');

    // var sliderOffsetX = slider.getBoundingClientRect().left - container.getBoundingClientRect().left;
    // var sliderOffsetY = slider.getBoundingClientRect().top - container.getBoundingClientRect().top;
