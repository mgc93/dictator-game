...
jsPsych.plugins['html-slider-response-dg-exact'] = (function() {

  var plugin = {};

  plugin.info = {
    name: 'html-slider-response-dg-exact',
    description: '',
    parameters: {
      stimulus: {
  ...
  plugin.trial = function(display_element, trial) {
    ...
    html += '">';
    html += '<input type="range" value="'+trial.slider_start+'" min="'+trial.min+'" max="'+trial.max+'" step="'+trial.step+'" style="width: 100%;" class="range"; id="jspsych-html-slider-response-response"></input>';
    html += '<output class="bubble-bottom"></output>';
    html += '<output class="bubble-top"></output>';
    html += '<div>'
    ...
    for(var j=0; j < trial.labels_top.length; j++){
      ...
      html += '<div style="display: inline-block; position: absolute; left:'+left_offset+'%; text-align: center; width: '+width+'%; top: -14%;">';
    ...
    for(var j=0; j < trial.labels_bottom.length; j++){
      ...
      html += '<div style="display: inline-block; position: absolute; left:'+left_offset+'%; text-align: center; width: '+width+'%; top: 12%;">';
    ...
    const allRanges = display_element.querySelectorAll(".jspsych-html-slider-response-container");
    allRanges.forEach(wrap => {
        const range = wrap.querySelector(".range");
        const bubble_bottom = wrap.querySelector(".bubble-bottom");
        const bubble_top = wrap.querySelector(".bubble-top");
        range.addEventListener("input", () => {
            setBubble(range, bubble_bottom, 1);
            setBubble(range, bubble_top, 2);
        });
            setBubble(range, bubble_bottom, 1);
            setBubble(range, bubble_top, 2);
    });
